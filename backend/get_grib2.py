...
def process_frame(frame_idx: int) -> bool:
    ...
    # Search queries grouped to prevent cfgrib index clashes
    search_patterns = [
    ...
    for pattern in search_patterns:
        try:
            # Fetch byte-range subset without writing local index locks
            ...
            for chunk_ds in dataset_list:
                ...
                for target_var in ds_cropped.data_vars:
                    ...
                    # 1. Wind U component (u10, u, u10m) - Keep in m/s for RGBA vector encoding
                    if var_lower in ['u10', 'u10m', 'u']:
                        ds_gfs['u10'] = ds_cropped[target_var].squeeze()

                    # 2. Wind V component (v10, v10m, v) - Keep in m/s for RGBA vector encoding
                    elif var_lower in ['v10', 'v10m', 'v']:
                        ds_gfs['v10'] = ds_cropped[target_var].squeeze()

                    # 3. Wind Gust
                    elif 'gust' in var_lower:
                        ds_gfs['gust'] = ds_cropped[target_var].squeeze() * 3.6  # m/s to km/h

                    # 4. Precipitation Rate
                    elif 'prate' in var_lower:
                        ds_gfs['prate'] = ds_cropped[target_var].squeeze() * 3600  # kg/m^2/s to mm/h

                    # 5. Temperature 2m (tmp, t2m, t, 2t)
                    ...
                    # 6. Relative Humidity 2m (rh, r2, r, 2r)
    ...
    # Fallback for missing precipitation (Frame 0)
    ...
    # Sort coordinates ascending before spline interpolation
    ...
    # Apply Gaussian Filter kernel
    ds_smoothed = apply_gaussian_smoothing(ds_smoothed)

    # Reverse latitude back to North-to-South (top-down) for image raster rendering
